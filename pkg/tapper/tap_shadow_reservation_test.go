...
// WithNodeLock. Before the fix this existence gate used Repo.HasNode,
// which returns true for shadow reservations, and the call would proceed
// to return empty metadata as if the node were real.
...
// open a bare node directory for editing. The dangerous pre-fix behaviour
// was to pass the existence gate and then attempt to read content from a
// directory with no README.md — leading to an empty-body edit session
// that would create content on a node the user never Created.
...
// refuses to attach a file to a shadow reservation. The pre-fix bug was
// the most dangerous of the five existence-gate sites: concurrent file
// upload against a bare directory would silently create attachments on a
// contentless node.
...
// of TestUploadFile_ShadowReservationRejected.
...
// issue a cross-process lock token against a shadow reservation. The
// pre-fix bug allowed acquiring a lock against a node that did not
// actually exist — an authentication hole against a nonexistent target.
