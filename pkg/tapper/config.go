...
type KegTarget struct {
	...
	//   - "alias"    : resolved alias from user config
...
func ResolveKegTargetForRepo(repoRoot string) (*KegTarget, error) {
	// Load user config once for the remaining precedence steps (mappings, aliases).
	cfg, _ := ReadUserConfig(ConfigAppName)
	...
	if repoRoot != "" {
		if val, err := gitLocalConfigGet(repoRoot, "tap.keg"); err == nil && val != "" {
			return &KegTarget{Value: val, Source: "git"}, nil
		}
	}
...
func (cfg *UserConfig) ResolveKegAlias(alias string) (KegTarget, error) {
	...
	return KegTarget{}, NewAliasNotFoundError(alias)
}
