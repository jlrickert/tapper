...
// TestList_ReflectsExternalWrites is a regression test for the stale-dex
// bug on long-lived Tap consumers (the MCP server). It verifies that
// Tap.List observes nodes written by another process between calls.
//
	...
	for _, want := range []string{idX, idY, idZ} {
		if _, ok := seen[want]; !ok {
			t.Fatalf("Tap A second List should reflect node %q created by Tap B "+
				"(stale-dex regression); got output:\n%s",
