...
func TestHookPreToolUse_GuardsCommands(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "reserved model assignment", command: "TAP_MODEL=other codex", deny: true},
		{name: "retired agent variable is ordinary", command: "export TAP_AGENT=other", deny: false},
